...
### TODO #17 — 실계정 아카이브 임포트 검증 + posts 기본키 충돌 버그 수정 + 검색 정렬 기능
완료. 결정 히스토리:
- **계기**: 사용자가 X 크레딧 충전 전에 "폴링보다 아카이브 임포트를 먼저 해야 하지 않냐"고 지적 → 폴링이 `sync_state` 부트스트랩 없이 시작되면 이미 무료로 받을 수 있는 데이터를 유료 API로 다시 받아오는 문제를 사전에 방지하는 게 맞다고 판단, 실제 X 데이터 아카이브(zip)로 첫 실사용 임포트 진행.
- **실행 방식**: 이 개발 환경(Windows)에 Docker가 없어 사용자가 "Docker 설치"를 요청했으나, WSL2 활성화에 관리자 권한(UAC 승격)이 필요해 이 세션에서는 자동화 불가 — Docker 없이 지금까지 써온 방식(SQLite 내장 모듈 + Meilisearch Windows 바이너리)으로 로컬 검증 진행. 업로드는 실제 프로덕션 경로(`import.html` → `POST /api/import/archive`)를 사용자가 브라우저에서 직접 수행(제가 대신 업로드하지 않음 — 본인 데이터라 업로드 주체를 사용자로 유지). 실제 아카이브가 106MB(part0)+4×105MB(part1~4)+13MB(part5)로 tweets가 6개 파일에 나뉘어 있었는데(X 아카이브 명명 규칙상 최초 파일은 `-part0` 접미사 없이 저장됨), 필요한 `data/tweets*.js`+`data/like.js`만 골라 zip으로 재구성(PowerShell `Compress-Archive`는 zip 내부 경로를 백슬래시로 저장해 코드가 기대하는 슬래시 경로와 안 맞는 버그가 있어, POSIX `zip` CLI로 전환).
- **발견한 심각한 버그**: `posts` 테이블이 트윗 id 단독을 기본키로 써서, 같은 트윗이 여러 relation으로 동시에 존재하는 경우(대표 사례: 본인 트윗을 본인이 좋아요) 나중에 저장되는 relation이 `INSERT OR IGNORE`/`ON CONFLICT DO NOTHING`에 조용히 무시됨. 실제 계정 데이터로 검증한 결과 `like.js` 3,817건 중 1,866건(약 49%)이 이 문제로 유실되어 있었음(전부 자가 좋아요 케이스로 확인). API 폴링 경로도 동일 구조라 아카이브 임포트만의 문제가 아님.
  - **수정**: SQLite/Postgres 스키마 기본키를 `id` 단독 → `(id, relation)` 복합키로 변경. `PostgresAdapter`의 `ON CONFLICT (id)` → `ON CONFLICT (id, relation)`. `getPostsPage()` 정렬을 `ORDER BY id` → `ORDER BY id, relation`(재색인 페이지네이션 안정성, `id` 단독으로는 더 이상 유일하지 않으므로).
  - Meilisearch는 인덱스 `primaryKey`가 불변이라, 합성 필드 `docId`(`` `${relation}_${id}` ``)를 새 primaryKey로 쓰도록 변경 — **콜론(`:`)은 Meilisearch 문서 id에 허용되지 않아** 첫 시도(`relation:id`)가 전량 실패했고(`invalid_document_id`), 언더스코어로 교체 후 정상 동작 확인. 기존 인덱스는 primaryKey 변경 불가라 삭제 후 재생성 필요, 로컬 DB도 스키마 변경분을 소급 반영할 수 없어 함께 삭제 후 재생성(이미 실계정 데이터가 들어있었지만, 이 로컬 DB는 실배포 서버와 별개의 개발 검증용이라 재생성 비용이 낮다고 판단, 재임포트로 온전히 복구됨).
  - Elasticsearch는 `_id`에 콜론 제약이 없어 `` `${relation}:${id}` `` 그대로 사용.
  - 프론트 `Feed.tsx`의 React `key`도 `hit.id` 단독이면 "전체" 뷰에서 같은 트윗이 여러 relation 카드로 동시에 노출될 때 충돌하므로 `` `${relation}-${id}` ``로 수정(상세는 `claude/rules/frontend.md` 참고).
  - **버그를 GitHub Issue로 별도 기록** — `gh` CLI가 이 환경에 없어 winget으로 설치했으나, `gh auth login --web`의 디바이스 코드 플로우가 반복적으로 완료되지 않아(원인 미확인, 방화벽/네트워크 추정) 결국 이슈 본문을 직접 작성해 사용자가 GitHub 웹에서 수동 등록.
- **정렬 기능 추가**: 검증 중 "좋아요 목록 날짜가 이상하다"는 사용자 지적에서 출발해, 검색 결과 정렬이 그동안 항상 검색엔진 관련도 랭킹이었고 명시적 정렬을 전혀 안 넘기고 있었다는 걸 발견. `SearchPagination`에 `sort?: "recency" | "relevance"` 추가(기본값 `recency`), Meilisearch는 `sort: ["createdAt:desc"]`/생략, Elasticsearch는 `sort: [{createdAt:"desc"}]`/생략으로 분기. `GET /api/search`에 `sort` 쿼리 파라미터 검증 추가.
  - **"관련도순이 반응 없다"는 버그 리포트 재현·조사**: Playwright로 실제 클릭 재현 결과, 요청 자체는 `sort=relevance`로 정상적으로 나가고 있었음 — 검색어 없이 "좋아요" 탭만 볼 때 결과 순서가 우연히 같아 보인 것으로 확인(검색어가 없으면 "관련도" 개념 자체가 성립하지 않아 Meilisearch가 기본 문서 순서로 폴백하는데, 아카이브의 `like.js` 저장 순서가 이미 최신순이라 우연히 일치). 실제 검색어를 넣으면 두 정렬이 명확히 다른 결과를 반환함을 curl로 확인 — 코드 버그 아님.
  - **좋아요 날짜 근원 조사**: X 공식 아카이브 뷰어(`Your archive.html`)가 좋아요를 시간순으로 보여주는 게 어떤 값 기준인지 사용자가 질문 → `like.js` 배열을 직접 파싱해 tweetId 순서를 검사한 결과 단순 tweetId 내림차순은 아니지만 전체적으로 최신→과거 흐름을 따름 — X가 "좋아요를 누른 실제 시각"을 내부적으로 갖고 있고 배열 순서에 암묵적으로 반영해 내보내는 것으로 추정(명시적 필드로는 제공 안 함). 최초 구현은 이 순서 정보를 활용하지 않고 임포트 시각만 씀 — 이후 아래 항목에서 개선.
- **좋아요 날짜 근사 개선 (같은 날 후속 작업)**: "좋아요 최신순이 실제로 적용된 거냐"는 사용자 질문에 원인(모든 좋아요가 동일한 임포트 시각을 `createdAt`으로 가져 사실상 동률)을 설명 → 부하 확인(대상 3,817건뿐, 이미 로컬에 있는 `like.js` 재파싱만 필요, 외부 API 호출 없음, 기존 `/api/admin/reindex` 재사용) 후 승인받아 진행.
  - `mapEntries.ts`의 `mapLikeEntry(raw, savedAt, orderIndex)`로 시그니처 변경 — `like.js` 배열 인덱스(0 = 가장 최근으로 간주)를 받아 `savedAt`에서 `orderIndex`초씩 차감한 합성 `createdAt`을 계산(실제 날짜는 아니고 상대적 정렬 전용값). `importArchive.ts`의 `importEntries`가 파일 전체를 관통하는 연속 인덱스를 세어 넘겨주도록 함(tweet 쪽 콜백은 인덱스 미사용).
  - 이미 임포트되어 있던 사용자의 실제 DB(3,817건)는 코드 수정만으로는 소급 반영되지 않아, 스크래치패드에 남아있던 원본 `like.js`를 재파싱해 같은 공식으로 `created_at`을 직접 `UPDATE`하는 1회성 스크립트를 작성해 실행(레포에는 커밋하지 않는 임시 스크립트). DB 파일에 두 커넥션이 동시에 쓰지 않도록 백엔드를 잠시 내렸다가 실행 후 재기동.
